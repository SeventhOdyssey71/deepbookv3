...
**Prebuilt binary (no Rust toolchain required).** Look up the latest version on the [releases page](https://github.com/MystenLabs/deepbookv3/releases) and download the archive for your platform.
...
curl -L "https://github.com/MystenLabs/deepbookv3/releases/download/predict-cli-${VERSION}/predict-cli-${VERSION}-${TARGET}.tar.gz" | tar -xz
...
```bash
git clone https://github.com/MystenLabs/deepbookv3
cd deepbookv3/predict-cli
cargo install --path .
```
...
`cargo fmt --check`, `cargo clippy -- -D warnings`, and 21 unit tests gate every PR via `.github/workflows/predict-cli-ci.yml`. Tests cover pricing identities, settled-payout rules, format edge cases, direction validation, and spend-cap behavior.

## Releasing

1. Bump `version` in `Cargo.toml`, commit.
2. `git tag predict-cli-v0.2.0 && git push origin predict-cli-v0.2.0`.
